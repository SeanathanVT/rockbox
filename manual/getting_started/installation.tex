...
\opt{innioasisy1}{
  Rockbox is installed on the \playertype{} by writing two firmware images to
  the player with a flashing tool; this replaces the original firmware. There
  is no automated installer for the \playertype{} yet, so follow the manual
  instructions below.\\
}
...
\opt{gigabeats,fiiom3k,shanlingq1,innioasisy1}{
...
\opt{innioasisy1}{
\item[The Rockbox firmware.]
  Most of the Rockbox code is contained in a ``build'' that, on the
  \playertype{}, is written to the player as a system image when you install
  Rockbox (it replaces the original firmware). The build consists of a
  directory called \fname{.rockbox} which contains all of the Rockbox files.
}
\end{description}
...
\opt{gigabeats,ondavx777,fiiom3k,shanlingq1,erosqnative,innioasisy1}{
...
\opt{innioasisy1}{\input{getting_started/innioasisy1_install.tex}}
...
\opt{swcodec}{%
    \nopt{gigabeats,erosqnative,rgnano,innioasisy1}{%
...
\opt{innioasisy1}{
  Once both images have been written, disconnect the USB cable and reset the
  \dap{}. It should boot straight into Rockbox.
}
...
\opt{innioasisy1}{
To update Rockbox, download a newer build and write its system image to the
\dap{} with \fname{mtkclient}, exactly as during installation
(\reference{sec:installing_firmware}). The boot image only changes rarely and
does not normally need to be rewritten.\\
}
...
\nopt{erosqnative,rgnano,innioasisy1}{
...
\section{Uninstalling Rockbox}\label{sec:uninstall}\index{Installation!uninstall}

\nopt{gigabeatf,iaudiom3,iaudiom5,iaudiox5,mrobe100,gigabeats,fuzeplus,erosqnative,rgnano,innioasisy1}{
...
\opt{innioasisy1}{
  To return to the original firmware, write the stock \fname{boot.img} and
  \fname{system.img} back to the \fname{BOOTIMG} and \fname{ANDROID}
  partitions with \fname{mtkclient}, in the same way as installing
  (\reference{sec:installing_firmware}). This requires a backup of the
  original images taken before installing Rockbox.
}
...
\nopt{gigabeats,rgnano,innioasisy1}{
